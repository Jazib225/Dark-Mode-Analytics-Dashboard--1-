...
export function Markets({ toggleBookmark, isBookmarked, onWalletClick, onMarketSelect, onBack, initialMarketId, initialMarketData }: MarketsProps) {
  ...
  // Create sorted lists for different columns
  const trendingMarkets = displayedMarkets; // Already trending
  
  // New Markets: Sort by newest (assuming markets are ordered newest first by API, or use mock timestamp)
  const newMarkets = [...allMarkets].sort((a, b) => {
    // If API doesn't provide created time, use the order they appear (newest markets are listed first by API)
    // For now, take the first N items assuming API returns them in order of recency
    return 0; // Keep original order (API returns newest first)
  }).slice(0, displayedCount);
  
  // Resolving Soon: Sort by resolution time (ascending - soonest first)
  // Note: Current API may not provide resolution time, so we'll note this in code
  const resolvingSoonMarkets = [...allMarkets].sort((a: any, b: any) => {
    // TODO: Once API provides resolutionDate or endTime, sort by that
    // For now, use reverse of trending (as fallback until API provides dates)
    return 0; // Keep original order until API provides resolution times
  }).slice(0, displayedCount);
...
// Dummy SearchBar component for time being - replace with actual if available
function SearchBar({ onSearch }: { onSearch: (query: string) => void }) {
  return (
    <input
      ...
      className="px-4 py-2 bg-slate-800 text-white border border-slate-700 rounded-lg hover:bg-slate-700 transition-colors text-sm"
    />
  );
}

// OLD CODE BELOW - KEEPING FOR REFERENCE DURING TRANSITION
/*
  return (
